import type { LandscapePhoto } from "@repo/core";
...
export function LandscapeShowcase({ photos: initialPhotos }: { photos: LandscapePhoto[] }) {
  const [selected, setSelected] = useState<LandscapePhoto | null>(null);
  ...
  const handleUpdate = useCallback((id: string, values: Record<string, unknown>) => {
    setPhotos((prev) =>
      prev.map((p) => (p.id === id ? ({ ...p, ...values } as LandscapePhoto) : p)),
    );
  }, []);

  return (
    <>
      <SectionCard title=" 拍摄风景图片展示" accent="bg-emerald-500">
        {photos.length === 0 ? (
          <p className="py-8 text-center text-xs text-muted-foreground">还没有上传风景照</p>
        ) : (
...
function PhotoThumb({ photo, onClick }: { photo: LandscapePhoto; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
function PhotoModal({ photo, onClose }: { photo: LandscapePhoto; onClose: () => void }) {
  return (
    <div
